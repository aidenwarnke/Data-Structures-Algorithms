...
def sum_series(lines_before_coffee, prod_loss):
  ...
  prod_loss_place = 1
  num = 0
  while lines_before_coffee // prod_loss_place > 0:
    num += (lines_before_coffee) // prod_loss_place
    
    prod_loss_place *= prod_loss
...
def linear_search(total_lines, prod_loss):
    ...
    initial_line = 0
    count = 0
    while sum_series(initial_line, prod_loss) < total_lines:
        initial_line += 1
        count += 1
    
    return initial_line, count
...
def binary_search(total_lines, prod_loss):
    count = 0
    low, high = 0, total_lines
    while low <= high:
        count += 1
        mid = (low + high) // 2
        if sum_series(mid, prod_loss) < total_lines:
            low = mid + 1
