...
class ImageGraph:
    ...
    def bfs(self, start_index, color):
        ...
        while not queue.is_empty():
            curr = queue.dequeue()
            for node in self.nodes[curr].edges:
                if not self.nodes[node].visited and self.nodes[node].color == start_color:
                    self.nodes[node].visit_and_set_color(color)
                    self.print_image()
                    queue.enqueue(node)
    ...
    def dfs(self, start_index, color):
        ...
        self.recursive(start_index, color, start_color)

    def recursive(self, start_index, color, start_color):
        if self.nodes[start_index].visited or self.nodes[start_index].color != start_color:
            return
        self.nodes[start_index].visit_and_set_color(color)
        self.print_image()

        for node in self.nodes[start_index].edges:
            self.recursive(node, color, start_color)
